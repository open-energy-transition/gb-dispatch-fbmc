...
def assign_bid_offer(
    ...
    bid_profiles = pd.DataFrame(np.select(conditions, bid_profiles), columns=interconnectors.index).add_suffix(" bid")
    offer_profiles = pd.DataFrame(np.select(conditions, offer_profiles), columns=interconnectors.index).add_suffix(" offer")

    interconnector_profile = pd.concat([bid_profiles, offer_profiles], axis=1)
    logger.info("Assigned the bid/offer to each interconnector based on the status of the interconnector")
...
if __name__ == "__main__":
    ...
    profile_names = ["import_bid", "import_offer", "float_import", "float_export", "export_bid", "export_offer"]
    
    profiles={f: pd.read_csv(x, index_col="snapshot", parse_dates=True) for f,x in zip(profile_names,snakemake.input.bid_offer_profiles)}
    
    interconnector_profile=assign_bid_offer(unconstrained_result, interconnectors, profiles)

    interconnector_profile.to_csv(snakemake.output.csv)
    logger.info(f"Exported CSV of interconnector bid/offer profiles to {snakemake.output.csv}")
